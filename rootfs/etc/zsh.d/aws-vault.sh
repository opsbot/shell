...
ASSUME_ROLE_INTERACTIVE="${ASSUME_ROLE_INTERACTIVE:-true}"

export AWS_DATA_PATH="${AWS_DATA_PATH:-/localhost/.aws}"
export AWS_CONFIG_FILE="${AWS_DATA_PATH}/config"

export AWS_VAULT_ASSUME_ROLE_TTL="${AWS_VAULT_ASSUME_ROLE_TTL:-1h}"
export AWS_VAULT_BACKEND="${AWS_VAULT_BACKEND:-file}"
export AWS_VAULT_ENABLED="${AWS_VAULT_ENABLED:-true}"
# export AWS_VAULT_PASS_CMD="${AWS_VAULT_PASS_CMD}"
# export AWS_VAULT_PASS_PREFIX="${AWS_VAULT_PASS_PREFIX}"
# export AWS_VAULT_PASS_PASSWORD_STORE_DIR="${AWS_VAULT_PASS_PASSWORD_STORE_DIR}"
export AWS_VAULT_PROMPT="${AWS_VAULT_PROMPT:-terminal}"
export AWS_VAULT_SECRET_SERVICE_COLLECTION_NAME="${AWS_VAULT_SECRET_SERVICE_COLLECTION_NAME:-awsvault}"
export AWS_VAULT_SERVER_ENABLED="${AWS_VAULT_SERVER_ENABLED:-false}"
export AWS_VAULT_SESSION_TTL="${AWS_VAULT_SESSION_TTL:-12h}"
...
assume_active_aws_role() {
	...
	if [ -n "${TF_VAR_aws_assume_role_arn}" ]
  then
    ...
    then
		...
		if [[ -z $AWS_VAULT || $AWS_VAULT = $aws_vault ]]
    then
...
then
  ...
  then
    ...
    then
		...
		if [[ -n $AWS_VAULT_ASSUME_ROLE_TTL ]]
    then
      ...
      then
		...
		if aws-vault exec --help 2>&1 | grep -q -e --duration
    then # aws-vault version 5
			if [[ -n $AWS_VAULT_SESSION_TTL ]]
      then
      ...
      then
		...
		elif [[ -n $AWS_VAULT_SESSION_TTL ]]
    then # aws-vault version 4
			# AWS_VAULT_SESSION_TTL takes priority
	...
	aws_vault_prompt() {
		[ -z "${AWS_VAULT}" ] && echo -e "-> Run '$(green assume-role)' to login to AWS with aws-vault"
	}
	...
	choose_role() {
		if [ "$ASSUME_ROLE_INTERACTIVE" = "true" ]
    then
    ...
    then
			...
			if [ "$SHLVL" -eq 1 ] && [ "$AWS_VAULT_SERVER_ENABLED" = "true" ]
      then
				# Save the current values of AWS_VAULT and AWS_VAULT_SERVER_ENABLED
				local aws_vault="$AWS_VAULT"
				local aws_vault_server_enabled="$AWS_VAULT_SERVER_ENABLED"
    ...
    then
		...
		if [ $# -eq 0 ]
    then
			aws-vault exec ${AWS_VAULT_ARGS[@]} $role -- /bin/zsh --login
		else
  		shift
